...
export default function ScannerLoader({ emoji = "🔍" }: ScannerLoaderProps) {
  return (
    <div className="relative h-50 w-50 rounded-2xl overflow-hidden bg-white/3 border border-white/8 shrink-0">
      <ScanCorners color={theme.hex.primary600} size={24} thickness={2} />

      {/* Animated scan line - back and forth */}
      <motion.div
        className="absolute left-0 right-0 h-px bg-linear-to-r from-transparent via-primary-400 to-transparent"
        style={{
      ...
      <div
        className="absolute left-0 right-0 h-0.5 bg-linear-to-r from-transparent via-primary-400 to-transparent animate-scan-line"
        style={{
